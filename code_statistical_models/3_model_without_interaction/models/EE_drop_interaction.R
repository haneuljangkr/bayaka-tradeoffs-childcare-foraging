...
pp_check(m1_prior_EE_age, ndraws = 50) + 
  scale_x_continuous(limits = c(0,1.5)) +
  theme_bw(base_size = 18)

pp_check(m1_prior_EE_kin, ndraws = 50) + 
  scale_x_continuous(limits = c(0,1.5)) +
  theme_bw(base_size = 18)

pp_check(m1_prior_EE_sex, ndraws = 50) + 
  scale_x_continuous(limits = c(0,1.5)) +
  theme_bw(base_size = 18)
...
m1_EE_female_age <- brm(EE_s ~
                           ...
                           control = list(adapt_delta = 0.98),
                           init = "0")
summary(m1_EE_female_age)

m1_EE_nursing <- brm(EE_s ~
                          # focalID-level predictors (fixed effects)
                        ...
                        control = list(adapt_delta = 0.98),
                        init = "0")
summary(m1_EE_nursing)

save.image("EE_drop_interaction.RData")

##########################
### Model comparison #####
##########################
m1_EE_0 <- add_criterion(m1_EE_0, criterion = "loo")
m1_EE_age <- add_criterion(m1_EE_age, criterion = "loo") # moment_match helps with influential observations, omitting that step for no
m1_EE_kin <- add_criterion(m1_EE_kin, criterion = "loo") # moment_match helps with influential observations, omitting that step for no
m1_EE_sex <- add_criterion(m1_EE_sex, criterion = "loo") # moment_match helps with influential observations, omitting that step for no

loo::loo_compare(loo(m1_EE_0), loo(m1_EE_age))
loo::loo_compare(loo(m1_EE_0), loo(m1_EE_kin))
loo::loo_compare(loo(m1_EE_0), loo(m1_EE_sex))
...
loo::loo_model_weights(list(m0 = loo(m1_EE_0), m_age = loo(m1_EE_age)))
loo::loo_model_weights(list(m0 = loo(m1_EE_0), m_kin = loo(m1_EE_kin)))
loo::loo_model_weights(list(m0 = loo(m1_EE_0), m_sex = loo(m1_EE_sex)))
...
pp_check(m1_EE_age, type = "dens_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m1_EE_kin, type = "dens_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m1_EE_sex, type = "dens_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].

# Empirical cumulative density function
pp_check(m1_EE_age, type = "ecdf_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m1_EE_kin, type = "ecdf_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m1_EE_sex, type = "ecdf_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].

# check for focal Id 
pp_check(m1_EE_age, type = "dens_overlay_grouped", group = "focalID") + scale_x_continuous(limits = c(0,1))

# ECDF plots for each focal woman 
pp_check(m1_EE_age, type = "ecdf_overlay_grouped", group = "focalID") + scale_x_continuous(limits = c(0,1))

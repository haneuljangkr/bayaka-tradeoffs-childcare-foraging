...
m1_prior_range_age <- brm(range_s ~
...
m1_prior_range_kin <- brm(range_s ~
                           # focalID-level predictors (fixed effects)
...
m1_prior_range_sex <- brm(range_s ~
                           # focalID-level predictors (fixed effects)
...
pp_check(m1_prior_range_age, ndraws = 50) + 
  scale_x_continuous(limits = c(0,1.5)) +
  theme_bw(base_size = 18)

pp_check(m1_prior_range_kin, ndraws = 50) + 
  scale_x_continuous(limits = c(0,1.5)) +
  theme_bw(base_size = 18)

pp_check(m1_prior_range_sex, ndraws = 50) + 
  scale_x_continuous(limits = c(0,1.5)) +
  theme_bw(base_size = 18)
...
m1_range_age <- brm(range_s ~
                   ...
                   control = list(adapt_delta = 0.98),
                   init = "0")
summary(m1_range_age)

m1_range_kin <- brm(range_s ~
                     # focalID-level predictors (fixed effects)
                     avg_prop_infant + avg_num_kin + avg_num_nonkin +
                     # trip-level predictors (fixed effects) 
                     infant_presence + n_kin_c + n_nonkin_c +
                   ...
                   control = list(adapt_delta = 0.98),
                   init = "0")
summary(m1_range_kin)

m1_range_sex <- brm(range_s ~
                     # focalID-level predictors (fixed effects)
                     avg_prop_infant + avg_num_females + avg_num_males +
                     # trip-level predictors (fixed effects) 
                     infant_presence + n_females_c + n_males_c +
                   ...
                   control = list(adapt_delta = 0.98),
                   init = "0")
summary(m1_range_sex)

m1_range_female_age <- brm(range_s ~
                           # focalID-level predictors (fixed effects)
                         ...
                         control = list(adapt_delta = 0.98),
                         init = "0")
summary(m1_range_female_age)

m1_range_nursing <- brm(range_s ~
                        # focalID-level predictors (fixed effects)
                      ...
                      control = list(adapt_delta = 0.98),
                      init = "0")
summary(m1_range_nursing)

save.image(file = "range_drop_interaction.RData") 

##########################
### Model comparison #####
##########################
m1_range_0 <- add_criterion(m1_range_0, criterion = "loo")
m1_range_age <- add_criterion(m1_range_age, criterion = "loo") # moment_match helps with influential observations, omitting that step for no
m1_range_kin <- add_criterion(m1_range_kin, criterion = "loo") # moment_match helps with influential observations, omitting that step for no
m1_range_sex <- add_criterion(m1_range_sex, criterion = "loo") # moment_match helps with influential observations, omitting that step for no

loo::loo_compare(loo(m1_range_0), loo(m1_range_age))
loo::loo_compare(loo(m1_range_0), loo(m1_range_kin))
loo::loo_compare(loo(m1_range_0), loo(m1_range_sex))
...
loo::loo_model_weights(list(m0 = loo(m1_range_0), m_age = loo(m1_range_age)))
loo::loo_model_weights(list(m0 = loo(m1_range_0), m_kin = loo(m1_range_kin)))
loo::loo_model_weights(list(m0 = loo(m1_range_0), m_sex = loo(m1_range_sex)))

###########################################
###### Posterior predictive checks ########
###########################################
#The dark line is the observed distribution whereas the light lines are draws from the posterior.
pp_check(m1_range_age, type = "dens_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m1_range_kin, type = "dens_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m1_range_sex, type = "dens_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].

# Empirical cumulative density function
pp_check(m1_range_age, type = "ecdf_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m1_range_kin, type = "ecdf_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m1_range_sex, type = "ecdf_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].

# check for focal Id
pp_check(m1_range_age, type = "dens_overlay_grouped", group = "focalID") + scale_x_continuous(limits = c(0,1))

# ECDF plots for each focal woman 
pp_check(m1_range_age, type = "ecdf_overlay_grouped", group = "focalID") + scale_x_continuous(limits = c(0,1))

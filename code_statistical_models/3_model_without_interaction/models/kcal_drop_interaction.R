...
pp_check(m1_prior_kcal_age, ndraws = 50) + 
  scale_x_continuous(limits = c(0,1.5)) +
  theme_bw(base_size = 18)

pp_check(m1_prior_kcal_kin, ndraws = 50) + 
  scale_x_continuous(limits = c(0,1.5)) +
  theme_bw(base_size = 18)

pp_check(m1_prior_kcal_sex, ndraws = 50) + 
  scale_x_continuous(limits = c(0,1.5)) +
  theme_bw(base_size = 18)
...
m1_kcal_female_age <- brm(kcal_s ~
                        ...
                        control = list(adapt_delta = 0.98),
                        init = "0")
summary(m1_kcal_female_age)

m1_kcal_nursing <- brm(kcal_s ~
                       # focalID-level predictors (fixed effects)
                     ...
                     control = list(adapt_delta = 0.98),
                     init = "0")
summary(m1_kcal_nursing)

save.image("kcal_drop_interaction.RData")

##########################
### Model comparison #####
##########################
m1_kcal_0 <- add_criterion(m1_kcal_0, criterion = "loo")
m1_kcal_age <- add_criterion(m1_kcal_age, criterion = "loo") # moment_match helps with influential observations, omitting that step for no
m1_kcal_kin <- add_criterion(m1_kcal_kin, criterion = "loo") # moment_match helps with influential observations, omitting that step for no
m1_kcal_sex <- add_criterion(m1_kcal_sex, criterion = "loo") # moment_match helps with influential observations, omitting that step for no

loo::loo_compare(loo(m1_kcal_0), loo(m1_kcal_age))
loo::loo_compare(loo(m1_kcal_0), loo(m1_kcal_kin))
loo::loo_compare(loo(m1_kcal_0), loo(m1_kcal_sex))
...
loo::loo_model_weights(list(m0 = loo(m1_kcal_0), m_age = loo(m1_kcal_age)))
loo::loo_model_weights(list(m0 = loo(m1_kcal_0), m_kin = loo(m1_kcal_kin)))
loo::loo_model_weights(list(m0 = loo(m1_kcal_0), m_sex = loo(m1_kcal_sex)))

###########################################
###### Posterior predictive checks ########
###########################################
#The dark line is the observed distribution whereas the light lines are draws from the posterior.
pp_check(m1_kcal_age, type = "dens_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m1_kcal_kin, type = "dens_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m1_kcal_sex, type = "dens_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].

# Empirical cumulative density function
pp_check(m1_kcal_age, type = "ecdf_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m1_kcal_kin, type = "ecdf_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m1_kcal_sex, type = "ecdf_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].

# check for focal Id 
pp_check(m1_kcal_age, type = "dens_overlay_grouped", group = "focalID") + scale_x_continuous(limits = c(0,1))

# ECDF plots for each focal woman 
pp_check(m1_kcal_age, type = "ecdf_overlay_grouped", group = "focalID") + scale_x_continuous(limits = c(0,1))

...
d1 <- d %>% 
  filter(!(is.na(maximum_distance)))

# For distance model, we only consider when they made foraging trips to the forest, as garden as fixed locations with the same distances always
d1 <- d1 %>% 
  filter(locID!="garden") 
table(d1$locID)
...
m_prior_max_age <- brm(max_dist_s ~
...
m_prior_max_kin <- brm(max_dist_s ~
                        # focalID-level predictors (fixed effects)
...
m_prior_max_sex <- brm(max_dist_s ~
                        # focalID-level predictors (fixed effects)
...
pp_check(m_prior_max_age, ndraws = 50) + 
  scale_x_continuous(limits = c(0,1.5)) +
  theme_bw(base_size = 18)

pp_check(m_prior_max_kin, ndraws = 50) + 
  scale_x_continuous(limits = c(0,1.5)) +
  theme_bw(base_size = 18)

pp_check(m_prior_max_sex, ndraws = 50) + 
  scale_x_continuous(limits = c(0,1.5)) +
  theme_bw(base_size = 18)
...
save.image(file="max_distance_interaction.RData")

##########################
### Model comparison #####
##########################
m_max_0 <- add_criterion(m_max_0, criterion = "loo")
m_max_age <- add_criterion(m_max_age, criterion = "loo") # moment_match helps with influential observations, omitting that step for no
m_max_kin <- add_criterion(m_max_kin, criterion = "loo") # moment_match helps with influential observations, omitting that step for no
m_max_sex <- add_criterion(m_max_sex, criterion = "loo") # moment_match helps with influential observations, omitting that step for no

loo::loo_compare(loo(m_max_0), loo(m_max_age))
loo::loo_compare(loo(m_max_0), loo(m_max_kin))
loo::loo_compare(loo(m_max_0), loo(m_max_sex))
...
loo::loo_model_weights(list(m0 = loo(m_max_0), m_age = loo(m_max_age)))
loo::loo_model_weights(list(m0 = loo(m_max_0), m_kin = loo(m_max_kin)))
loo::loo_model_weights(list(m0 = loo(m_max_0), m_sex = loo(m_max_sex)))

###########################################
###### Posterior predictive checks ########
###########################################
#The dark line is the observed distribution whereas the light lines are draws from the posterior.
pp_check(m_max_age, type = "dens_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m_max_kin, type = "dens_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m_max_sex, type = "dens_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].

# Empirical cumulative density function
pp_check(m_max_age, type = "ecdf_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m_max_kin, type = "ecdf_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m_max_sex, type = "ecdf_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].

# We can also check the model predictions by group witin the data, for example whether infants were present or not
pp_check(m_max_age, type = "dens_overlay_grouped", group = "infant_presence") + scale_x_continuous(limits = c(0,1))
pp_check(m_max_kin, type = "dens_overlay_grouped", group = "infant_presence") + scale_x_continuous(limits = c(0,1))
pp_check(m_max_sex, type = "dens_overlay_grouped", group = "infant_presence") + scale_x_continuous(limits = c(0,1))

# per each focal woman
pp_check(m_max_age, type = "dens_overlay_grouped", group = "focalID") + scale_x_continuous(limits = c(0,1))

# ECDF plots for each focal woman 
pp_check(m_max_age, type = "ecdf_overlay_grouped", group = "focalID") + scale_x_continuous(limits = c(0,1))
...
pred_adults <- posterior_epred(m_max_age, newdata = newdata_adults_max, re.form = NA)
...
pred_adults_long <- as.data.frame(pred_adults) %>% 
  ...
  mutate(pred_max = pred * max(d1$maximum_distance, na.rm=T))
...
pred_adults_summary <- pred_adults_long %>% 
  group_by(condition) %>% 
  summarise(med = median(pred_max), infant_presence = unique(infant_presence), n_adults_c = unique(n_adults_c))
...
pred_children <- posterior_epred(m_max_age, newdata = newdata_children_max, re.form = NA)
...
pred_children_summary <- pred_children_long %>% 
  group_by(condition) %>% 
  summarise(med = median(pred_max), infant_presence = unique(infant_presence), n_children_c = unique(n_children_c))

###### OK, lets plot both #########
library(patchwork)

p_max_adults <- ggplot( data = filter(pred_adults_long, samp <= 100), aes(x = n_adults_c, y = pred_max, color = infant_presence, group = factor(samp)) ) +
...
p_max_children <- ggplot( data = filter(pred_children_long, samp <= 100), aes(x = n_children_c, y = pred_max, color = infant_presence, group = factor(samp)) ) +
  facet_wrap(~infant_presence) +
  geom_line(alpha = 0.3, lwd = 0.3) +
...
p_max_adults / p_max_children

# We can also plot the difference betwmaxn conditions (infant present/absent)
pred_adults_diff <- pred_adults_long %>% 
  group_by(samp, n_adults_c) %>% 
  summarise(diff = pred_max[infant_presence == "infant present"] - pred_max[infant_presence == "infant absent"]) 
...
p_diff_adults <- ggplot( data = filter(pred_adults_diff, samp <= 100), aes(x = n_adults_c, y = diff, group = factor(samp)) ) +
  ...
  theme_minimal(base_size = 18) +
  ylab("Diff (m) [Infant present - Infant absent]") +
  xlab("Number of adults (z-score)") +
  theme(legend.position = "none", panel.spacing = unit(2, "lines"))
...
pred_kin <- posterior_epred(m_max_kin, newdata = newdata_kin_max, re.form = NA)
...
pred_kin_summary <- pred_kin_long %>% 
  group_by(condition) %>% 
  summarise(med = median(pred_max), infant_presence = unique(infant_presence), n_kin_c = unique(n_kin_c))
...
pred_nonkin <- posterior_epred(m_max_kin, newdata = newdata_nonkin_max, re.form = NA)
...
pred_nonkin_summary <- pred_nonkin_long %>% 
  group_by(condition) %>% 
  summarise(med = median(pred_max), infant_presence = unique(infant_presence), n_nonkin_c = unique(n_nonkin_c))

###### OK, lets plot both #########
library(patchwork)

p_max_kin <- ggplot( data = filter(pred_kin_long, samp <= 100), aes(x = n_kin_c, y = pred_max, color = infant_presence, group = factor(samp)) ) +
  facet_wrap(~infant_presence) +
  geom_line(alpha = 0.3, lwd = 0.3) +
...
p_max_nonkin <- ggplot( data = filter(pred_nonkin_long, samp <= 100), aes(x = n_nonkin_c, y = pred_max, color = infant_presence, group = factor(samp)) ) +
  facet_wrap(~infant_presence) +
  geom_line(alpha = 0.3, lwd = 0.3) +
...
pred_females <- posterior_epred(m_max_sex, newdata = newdata_females_max, re.form = NA)
...
pred_females_summary <- pred_females_long %>% 
  group_by(condition) %>% 
  summarise(med = median(pred_max), infant_presence = unique(infant_presence), n_females_c = unique(n_females_c))
...
pred_males <- posterior_epred(m_max_sex, newdata = newdata_males_max, re.form = NA)
...
pred_males_summary <- pred_males_long %>% 
  group_by(condition) %>% 
  summarise(med = median(pred_max), infant_presence = unique(infant_presence), n_males_c = unique(n_males_c))

###### OK, lets plot both #########
library(patchwork)

p_max_females <- ggplot( data = filter(pred_females_long, samp <= 100), aes(x = n_females_c, y = pred_max, color = infant_presence, group = factor(samp)) ) +
  facet_wrap(~infant_presence) +
  geom_line(alpha = 0.3, lwd = 0.3) +
...
p_max_males <- ggplot( data = filter(pred_males_long, samp <= 100), aes(x = n_males_c, y = pred_max, color = infant_presence, group = factor(samp)) ) +
  facet_wrap(~infant_presence) +
  geom_line(alpha = 0.3, lwd = 0.3) +

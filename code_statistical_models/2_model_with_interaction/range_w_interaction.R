...
d1 <- d %>% 
  filter(!(is.na(trip_range_MCP)))

# For range model, we only consider when they made foraging trips to the forest, as garden as fixed locations with the same distances always
d1 <- d1 %>% 
  filter(locID!="garden") 
table(d1$locID)
...
m_prior_range_age <- brm(range_s ~
                           ...
                           # additional random effect structure, I left out groupID for now due to problems with convergence
...
pp_check(m_prior_range_age, ndraws = 50) + 
  scale_x_continuous(limits = c(0,1.5)) +
  theme_bw(base_size = 18)

pp_check(m_prior_range_kin, ndraws = 50) + 
  scale_x_continuous(limits = c(0,1.5)) +
  theme_bw(base_size = 18)

pp_check(m_prior_range_sex, ndraws = 50) + 
  scale_x_continuous(limits = c(0,1.5)) +
  theme_bw(base_size = 18)
...
save.image(file = "range_interaction.RData") 

##########################
### Model comparison #####
##########################
m_range_0 <- add_criterion(m_range_0, criterion = "loo")
m_range_age <- add_criterion(m_range_age, criterion = "loo") # moment_match helps with influential observations, omitting that step for no
m_range_kin <- add_criterion(m_range_kin, criterion = "loo") # moment_match helps with influential observations, omitting that step for no
m_range_sex <- add_criterion(m_range_sex, criterion = "loo") # moment_match helps with influential observations, omitting that step for no

loo::loo_compare(loo(m_range_0), loo(m_range_age))
loo::loo_compare(loo(m_range_0), loo(m_range_kin))
loo::loo_compare(loo(m_range_0), loo(m_range_sex))
...
loo::loo_model_weights(list(m0 = loo(m_range_0), m_age = loo(m_range_age)))
loo::loo_model_weights(list(m0 = loo(m_range_0), m_kin = loo(m_range_kin)))
loo::loo_model_weights(list(m0 = loo(m_range_0), m_sex = loo(m_range_sex)))

###########################################
###### Posterior predictive checks ########
###########################################
#The dark line is the observed distribution whereas the light lines are draws from the posterior.
pp_check(m_range_age, type = "dens_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m_range_kin, type = "dens_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m_range_sex, type = "dens_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].

# Empirical cumulative density function
pp_check(m_range_age, type = "ecdf_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m_range_kin, type = "ecdf_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m_range_sex, type = "ecdf_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].

# We can also check the model predictions by group witin the data, for example whether infants were present or not
pp_check(m_range_age, type = "dens_overlay_grouped", group = "infant_presence") + scale_x_continuous(limits = c(0,1))
pp_check(m_range_kin, type = "dens_overlay_grouped", group = "infant_presence") + scale_x_continuous(limits = c(0,1))
pp_check(m_range_sex, type = "dens_overlay_grouped", group = "infant_presence") + scale_x_continuous(limits = c(0,1))

# check for focal Id
pp_check(m_range_age, type = "dens_overlay_grouped", group = "focalID") + scale_x_continuous(limits = c(0,1))

# ECDF plots for each focal woman 
pp_check(m_range_age, type = "ecdf_overlay_grouped", group = "focalID") + scale_x_continuous(limits = c(0,1))
...
pred_adults <- posterior_epred(m_range_age, newdata = newdata_adults_range, re.form = NA)
...
pred_adults_summary <- pred_adults_long %>% 
  group_by(condition) %>% 
  summarise(med = median(pred_range), infant_presence = unique(infant_presence), n_adults_c = unique(n_adults_c))
...
pred_children <- posterior_epred(m_range_age, newdata = newdata_children_range, re.form = NA)
...
pred_children_summary <- pred_children_long %>% 
  group_by(condition) %>% 
  summarise(med = median(pred_range), infant_presence = unique(infant_presence), n_children_c = unique(n_children_c))

###### OK, lets plot both #########
library(patchwork)

p_range_adults <- ggplot( data = filter(pred_adults_long, samp <= 100), aes(x = n_adults_c, y = pred_range, color = infant_presence, group = factor(samp)) ) +
...
p_range_children <- ggplot( data = filter(pred_children_long, samp <= 100), aes(x = n_children_c, y = pred_range, color = infant_presence, group = factor(samp)) ) +
  facet_wrap(~infant_presence) +
  geom_line(alpha = 0.3, lwd = 0.3) +
...
p_range_adults / p_range_children

# We can also plot the difference betwrangen conditions (infant present/absent)
pred_adults_diff <- pred_adults_long %>% 
  group_by(samp, n_adults_c) %>% 
  summarise(diff = pred_range[infant_presence == "infant present"] - pred_range[infant_presence == "infant absent"]) 
...
p_diff_adults <- ggplot( data = filter(pred_adults_diff, samp <= 100), aes(x = n_adults_c, y = diff, group = factor(samp)) ) +
  ...
  theme_minimal(base_size = 18) +
  ylab("Diff (ha) [Infant present - Infant absent]") +
  xlab("Number of adults (z-score)") +
  theme(legend.position = "none", panel.spacing = unit(2, "lines"))
...
pred_kin <- posterior_epred(m_range_kin, newdata = newdata_kin_range, re.form = NA)
...
pred_kin_summary <- pred_kin_long %>% 
  group_by(condition) %>% 
  summarise(med = median(pred_range), infant_presence = unique(infant_presence), n_kin_c = unique(n_kin_c))
...
pred_nonkin <- posterior_epred(m_range_kin, newdata = newdata_nonkin_range, re.form = NA)
...
pred_nonkin_summary <- pred_nonkin_long %>% 
  group_by(condition) %>% 
  summarise(med = median(pred_range), infant_presence = unique(infant_presence), n_nonkin_c = unique(n_nonkin_c))

###### OK, lets plot both #########
library(patchwork)

p_range_kin <- ggplot( data = filter(pred_kin_long, samp <= 100), aes(x = n_kin_c, y = pred_range, color = infant_presence, group = factor(samp)) ) +
  facet_wrap(~infant_presence) +
  geom_line(alpha = 0.3, lwd = 0.3) +
...
p_range_nonkin <- ggplot( data = filter(pred_nonkin_long, samp <= 100), aes(x = n_nonkin_c, y = pred_range, color = infant_presence, group = factor(samp)) ) +
  facet_wrap(~infant_presence) +
  geom_line(alpha = 0.3, lwd = 0.3) +
...
pred_females <- posterior_epred(m_range_sex, newdata = newdata_females_range, re.form = NA)
...
pred_females_summary <- pred_females_long %>% 
  group_by(condition) %>% 
  summarise(med = median(pred_range), infant_presence = unique(infant_presence), n_females_c = unique(n_females_c))
...
pred_males <- posterior_epred(m_range_sex, newdata = newdata_males_range, re.form = NA)
...
pred_males_summary <- pred_males_long %>% 
  group_by(condition) %>% 
  summarise(med = median(pred_range), infant_presence = unique(infant_presence), n_males_c = unique(n_males_c))

###### OK, lets plot both #########
library(patchwork)

p_range_females <- ggplot( data = filter(pred_females_long, samp <= 100), aes(x = n_females_c, y = pred_range, color = infant_presence, group = factor(samp)) ) +
  facet_wrap(~infant_presence) +
  geom_line(alpha = 0.3, lwd = 0.3) +
...
p_range_males <- ggplot( data = filter(pred_males_long, samp <= 100), aes(x = n_males_c, y = pred_range, color = infant_presence, group = factor(samp)) ) +
  facet_wrap(~infant_presence) +
  geom_line(alpha = 0.3, lwd = 0.3) +

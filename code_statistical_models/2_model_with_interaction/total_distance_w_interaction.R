...
pp_check(m_prior_dist_age, ndraws = 50) + 
  scale_x_continuous(limits = c(0,1.5)) +
  theme_bw(base_size = 18)

pp_check(m_prior_dist_kin, ndraws = 50) + 
  scale_x_continuous(limits = c(0,1.5)) +
  theme_bw(base_size = 18)

pp_check(m_prior_dist_sex, ndraws = 50) + 
  scale_x_continuous(limits = c(0,1.5)) +
  theme_bw(base_size = 18)
...
save.image(file = "total_distance_interaction.RData") 

##########################
### Model comparison #####
##########################
m_dist_0 <- add_criterion(m_dist_0, criterion = "loo")
m_dist_age <- add_criterion(m_dist_age, criterion = "loo") # moment_match helps with influential observations, omitting that step for no
m_dist_kin <- add_criterion(m_dist_kin, criterion = "loo") # moment_match helps with influential observations, omitting that step for no
m_dist_sex <- add_criterion(m_dist_sex, criterion = "loo") # moment_match helps with influential observations, omitting that step for no

loo::loo_compare(loo(m_dist_0), loo(m_dist_age))
loo::loo_compare(loo(m_dist_0), loo(m_dist_kin))
loo::loo_compare(loo(m_dist_0), loo(m_dist_sex))
...
loo::loo_model_weights(list(m0 = loo(m_dist_0), m_age = loo(m_dist_age)))
loo::loo_model_weights(list(m0 = loo(m_dist_0), m_kin = loo(m_dist_kin)))
loo::loo_model_weights(list(m0 = loo(m_dist_0), m_sex = loo(m_dist_sex)))
...
pp_check(m_dist_age, type = "dens_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m_dist_kin, type = "dens_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m_dist_sex, type = "dens_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].

# Empirical cumulative density function
pp_check(m_dist_age, type = "ecdf_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m_dist_kin, type = "ecdf_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].
pp_check(m_dist_sex, type = "ecdf_overlay") + scale_x_continuous(limits = c(0,1)) # I put a limit on the x-axis because we scaled the data [0,1].

# We can also check the model predictions by group witin the data, for example whether infants were present or not
pp_check(m_dist_age, type = "dens_overlay_grouped", group = "infant_presence") + scale_x_continuous(limits = c(0,1))
pp_check(m_dist_kin, type = "dens_overlay_grouped", group = "infant_presence") + scale_x_continuous(limits = c(0,1))
pp_check(m_dist_sex, type = "dens_overlay_grouped", group = "infant_presence") + scale_x_continuous(limits = c(0,1))

# per each focal woman
pp_check(m_dist_age, type = "dens_overlay_grouped", group = "focalID") + scale_x_continuous(limits = c(0,1))

# ECDF plots for each focal woman 
pp_check(m_dist_age, type = "ecdf_overlay_grouped", group = "focalID") + scale_x_continuous(limits = c(0,1))
...
pred_adults <- posterior_epred(m_dist_age, newdata = newdata_adults_dist, re.form = NA)
...
pred_adults_summary <- pred_adults_long %>% 
  group_by(condition) %>% 
  summarise(med = median(pred_km), infant_presence = unique(infant_presence), n_adults_c = unique(n_adults_c))
...
pred_children <- posterior_epred(m_dist_age, newdata = newdata_children_dist, re.form = NA)
...
pred_children_summary <- pred_children_long %>% 
  group_by(condition) %>% 
  summarise(med = median(pred_km), infant_presence = unique(infant_presence), n_children_c = unique(n_children_c))

###### OK, lets plot both #########
library(patchwork)

p_dist_adults <- ggplot( data = filter(pred_adults_long, samp <= 100), aes(x = n_adults_c, y = pred_km, color = infant_presence, group = factor(samp)) ) +
...
p_dist_children <- ggplot( data = filter(pred_children_long, samp <= 100), aes(x = n_children_c, y = pred_km, color = infant_presence, group = factor(samp)) ) +
  facet_wrap(~infant_presence) +
  geom_line(alpha = 0.3, lwd = 0.3) +
...
pred_adults_diff <- pred_adults_long %>% 
  ...
  summarise(diff = pred_km[infant_presence == "infant present"] - pred_km[infant_presence == "infant absent"]) 
...
p_diff_adults <- ggplot( data = filter(pred_adults_diff, samp <= 100), aes(x = n_adults_c, y = diff, group = factor(samp)) ) +
  ...
  theme_minimal(base_size = 18) +
  ylab("Diff (km) [Infant present - Infant absent]") +
  xlab("Number of adults (z-score)") +
  theme(legend.position = "none", panel.spacing = unit(2, "lines"))
...
pred_kin <- posterior_epred(m_dist_kin, newdata = newdata_kin_dist, re.form = NA)
...
pred_kin_summary <- pred_kin_long %>% 
  group_by(condition) %>% 
  summarise(med = median(pred_km), infant_presence = unique(infant_presence), n_kin_c = unique(n_kin_c))
...
pred_nonkin <- posterior_epred(m_dist_kin, newdata = newdata_nonkin_dist, re.form = NA)
...
pred_nonkin_summary <- pred_nonkin_long %>% 
  group_by(condition) %>% 
  summarise(med = median(pred_km), infant_presence = unique(infant_presence), n_nonkin_c = unique(n_nonkin_c))

###### OK, lets plot both #########
library(patchwork)

p_dist_kin <- ggplot( data = filter(pred_kin_long, samp <= 100), aes(x = n_kin_c, y = pred_km, color = infant_presence, group = factor(samp)) ) +
  facet_wrap(~infant_presence) +
  geom_line(alpha = 0.3, lwd = 0.3) +
...
p_dist_nonkin <- ggplot( data = filter(pred_nonkin_long, samp <= 100), aes(x = n_nonkin_c, y = pred_km, color = infant_presence, group = factor(samp)) ) +
  facet_wrap(~infant_presence) +
  geom_line(alpha = 0.3, lwd = 0.3) +
...
pred_females <- posterior_epred(m_dist_sex, newdata = newdata_females_dist, re.form = NA)
...
pred_females_summary <- pred_females_long %>% 
  group_by(condition) %>% 
  summarise(med = median(pred_km), infant_presence = unique(infant_presence), n_females_c = unique(n_females_c))
...
pred_males <- posterior_epred(m_dist_sex, newdata = newdata_males_dist, re.form = NA)
...
pred_males_summary <- pred_males_long %>% 
  group_by(condition) %>% 
  summarise(med = median(pred_km), infant_presence = unique(infant_presence), n_males_c = unique(n_males_c))

###### OK, lets plot both #########
library(patchwork)

p_dist_females <- ggplot( data = filter(pred_females_long, samp <= 100), aes(x = n_females_c, y = pred_km, color = infant_presence, group = factor(samp)) ) +
  facet_wrap(~infant_presence) +
  geom_line(alpha = 0.3, lwd = 0.3) +
...
p_dist_males <- ggplot( data = filter(pred_males_long, samp <= 100), aes(x = n_males_c, y = pred_km, color = infant_presence, group = factor(samp)) ) +
  facet_wrap(~infant_presence) +
  geom_line(alpha = 0.3, lwd = 0.3) +
...
p_dist_females / p_dist_males

#ggsave("distkm_pred.pdf", height = 11, width = 8.5, dpi = 600)
